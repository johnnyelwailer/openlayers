...
function waitForWebGPU(callback) {
  const renderer = layer.getRenderer();
  const device =
    renderer &&
    /** @type {any} */ (renderer).helper &&
    /** @type {any} */ (renderer).helper.getDevice();
  if (device) {
    callback();
    return;
  }
  setTimeout(() => waitForWebGPU(callback), 50);
}
...
waitForWebGPU(() => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      render({
        message:
          'WebGPU vector tiles from lower zoom levels are hidden by higher zoom levels',
        tolerance: 0.001,
      });
    });
  });
